        ...
        var pageButtons = function(numberOfPage,currentPage) {
            ...
            for (let $i=1; $i<=numberOfPage;$i++){
                if(numberOfPage > 10){
                    ...
                    buttons += "<input type='button' id='id"+$i+"'value='"+$i+"' class='py-2 px-3 ml-0 leading-tight text-gray-600 bg-white border border-slate-500 hover:bg-gray-700 hover:text-white' onclick='paginate.sort("+$i+")'>";
                }
            }

            // let nextButton = "<input type='button' value='next →' class='paginate_control_next' onclick='paginate.sort("+(currentPage + 1)+")' "+nextDisabled+">";
            let nextButton = "<button href='#' class='py-2 px-3 leading-tight text-slate-600 bg-white rounded-r-lg border border-slate-500 hover:bg-gray-700 hover:text-white' onclick='paginate.sort("+(currentPage + 1)+")' "+nextDisabled+">Next</button>"
